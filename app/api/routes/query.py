...
@router.post(
    ...
    "and an answer generated by the LLM. Set expand=true to rewrite the query into "
    "variations and fuse results for broader retrieval.",
    responses={
...
async def query_endpoint(request: QueryRequest):
    try:
        return query_pipeline(request.question, request.top_k, expand=request.expand)
...
async def query_stream_endpoint(request: QueryRequest):
    try:
        if request.expand:
            vector_triplets = search_with_expansion(request.question, request.top_k)
        else:
